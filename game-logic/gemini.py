...
# -*- coding: utf-8 -*-
"""
Gemini OCR 11.3 – Sửa lỗi Copy và Hiển thị ảnh
Cải tiến:
  - Sửa lỗi copy không hoạt động bằng cách thêm self.update() và try-except.
  - Sửa dứt điểm lỗi không hiển thị ảnh preview bằng cách buộc cập nhật UI
    trước khi lấy kích thước widget (update_idletasks).
  - Bỏ cơ chế bắt sự kiện <Configure> không cần thiết.
"""
...
def load_cfg():
    default = {
        "api_key": "", "gmail": "", "model": "gemini-2.5-flash",
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Gemini OCR 11.3 – Sửa lỗi Copy & Ảnh")
        ...
        except Exception as e: print(f"Không thể đăng ký hotkey: {e}.")
    ...
    def open_settings(self):
        w = tk.Toplevel(self); w.title("Cài đặt – Gemini OCR"); w.geometry("1000x700"); w.resizable(True, True); w.transient(self); w.grab_set()
    ...
    # **SỬA 1**: Hàm cập nhật ảnh được đơn giản hóa và đáng tin cậy hơn
    ...
    def _process_ocr_input(self, b64_img):
        ...
        # **SỬA 2**: Buộc cập nhật giao diện TRƯỚC KHI hiển thị ảnh
        self.update_idletasks()
    ...
    # **SỬA 3**: Cải tiến hàm copy để đáng tin cậy hơn
    def copy_result(self):
        ...
        if text and "⏳" not in text and "❌" not in text:
            try:
                ...
                self.update() # Buộc cập nhật clipboard
                self.status.config(text="📋 Đã copy kết quả vào clipboard!")
            except tk.TclError:
                self.status.config(text="❌ Lỗi: Không thể truy cập clipboard.")
    ...
    def view_history(self):
        ...
        main_frame = ttk.Frame(win, padding=10); main_frame.pack(fill="both", expand=True)
        txt_frame = ttk.Frame(main_frame); txt_frame.pack(fill="both", expand=True)
        txt = tk.Text(txt_frame, wrap="word", font=("SF Mono", 11), padx=10, pady=10)
        scrollbar = ttk.Scrollbar(txt_frame, orient="vertical", command=txt.yview); scrollbar.pack(side="right", fill="y")
        txt.pack(side="left", fill="both", expand=True); txt.config(yscrollcommand=scrollbar.set)
        history = load_history()
        if not history: txt.insert("1.0", "Chưa có lịch sử OCR nào.")
        else:
            for item in history[:50]:
                ...
                txt.insert("end", header, "header"); txt.insert("end", f"{item['result']}\n\n", "result")
        txt.tag_config("header", foreground="#00FFFF", font=("SF Pro", 10, "bold")); txt.tag_config("result", lmargin1=20); txt.config(state="disabled")
        bottom_frame = ttk.Frame(main_frame, padding=(0, 10, 0, 0)); bottom_frame.pack(fill="x", side="bottom")
        clear_btn = ttk.Button(bottom_frame, text="🗑️ Xóa Toàn Bộ Lịch Sử", command=lambda: self.clear_history(win, txt)); clear_btn.pack(side="right")
    ...
    def sync_gmail_to_web(self, gmail: str):
        ...
        try:
            # **SỬA Ở ĐÂY**: Gói payload vào trong một dictionary có key là "data"
            requests.post("https://asia-southeast1-gamelogic4u.cloudfunctions.net/saveDevice", json={"data": payload}, timeout=10) 
