...
const AdminPage = () => {
    ...
    // Dán hàm này vào bên trong component AdminPage

const handleDeleteGroup = async (groupId, groupName) => {
    const result = await Swal.fire({
        title: `Bạn chắc chắn muốn xóa lớp "${groupName}"?`,
        text: "Hành động này sẽ xóa vĩnh viễn lớp học và bỏ gán giáo viên khỏi lớp. Không thể hoàn tác!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
        confirmButtonText: 'Vâng, xóa lớp này!',
        cancelButtonText: 'Hủy'
    });
    ...
    if (result.isConfirmed) {
        ...
        try {
            ...
            await deleteGroupFunc({ groupId: groupId });
            Swal.fire('Đã xóa!', `Lớp học "${groupName}" đã được xóa.`, 'success');
        } catch (error) {
    ...
    return (
        ...
        <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <h1 className="mb-0">Trang Quản Trị (Super Admin)</h1>
            <Link to="/admin/ocr" className="btn btn-info">
                <i className="fa-solid fa-camera me-2"></i>
                Quản lý OCR
            </Link>
        </div>
        {/* ✨ KẾT THÚC THAY ĐỔI ✨ */}
            {/* --- CÁC CARD CÀI ĐẶT VÀ QUẢN LÝ KHÁC GIỮ NGUYÊN --- */}
            <div className="card mb-4">
                <div className="card-body">
                    ...
                    <div className="form-check form-switch fs-5">
                        ...
                        <label className="form-check-label" htmlFor="approvalSwitch">Yêu cầu duyệt giáo viên mới</label>
                    </div>
                </div>
            </div>
            <div className="card mb-4">
            ...
            {/* --- ✨ CARD QUẢN LÝ LỚP HỌC (ĐÃ ĐƠN GIẢN HÓA) ✨ --- */}
            <div className="card mb-4">
                <div className="card-body">
                    <h5 className="card-title">Quản lý Lớp học</h5>
                    <p className="text-muted small">Lớp học giờ đây được tạo tự động khi giáo viên mới đăng ký. Dùng các nút xóa để dọn dẹp dữ liệu cũ.</p>
            ...
            {/* --- CARD QUẢN LÝ NGƯỜI DÙNG (CẢI TIẾN HIỂN THỊ) --- */}
            <div className="card">
                <div className="card-body">
                        ...
                        <table className="table table-hover align-middle">
                            <thead className="table-light">
                                <tr>
                                    ...
                                    <th>Lớp học</th>
                                    {/* Bỏ cột Admin Actions vì đã tích hợp vào cột Trạng thái */}
                                    <th className="text-end">Hành động</th>
                                </tr>
                            </thead>
                            <tbody>
