...
const QuizEditor = () => {
    ...
    const parseTxtToQuizJson = (txtContent) => {
        ...
        if (passwordMatch) quiz.password = passwordMatch[1].trim();
        const clusterBlocks = cleanContent.split('CUM_CAU_HOI_BAT_DAU');
        clusterBlocks.slice(1).forEach(block => {
            const cluster = { ...JSON.parse(JSON.stringify(emptyCluster)), questions: [] };
            const assumptionMatch = block.match(/GIA_THIET:\s*([\s\S]*?)(?=QUY_TAC:|CAU_HOI_BAT_DAU|CUM_CAU_HOI_KET_THUC)/);
            ...
            questionBlocks.slice(1).forEach(qBlock => {
                const question = { ...JSON.parse(JSON.stringify(emptyQuestion)), choices: [] };
                const getVal = (key) => {
                ...
                if (question.type === 'multiple_choice') {
                    ...
                    question.choices = choiceMatches.map(m => ({ value: m[1], text: m[2].trim(), imageUrl: null }));
                }
            ...
            if(cluster.questions.length > 0) quiz.clusters.push(cluster);
        });
        return quiz;
    };
    ...
    const handleUploadQuiz = async (event) => {
        ...
        if (!file) return;
        Swal.fire({ title: 'Đang xử lý file...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        try {
            let parsedData;
            ...
            Swal.fire('Thành công!', 'Đã tải và phân tích nội dung file!', 'success');
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        Swal.fire({ title: 'Đang lưu...', html: 'Vui lòng chờ trong giây lát.', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
            ...
            for (const cluster of quizToSave.clusters) {
                for (const question of cluster.questions) {
                    if (question.imageUrl && question.imageUrl.startsWith('blob:')) {
                        ...
                        if(originalQFile) question.imageUrl = await compressAndUploadImage(originalQFile);
                    }
                    ...
                    if (question.choices) {
                        for (const choice of question.choices) {
                            if (choice.imageUrl && choice.imageUrl.startsWith('blob:')) {
                                ...
                                if(originalChFile) choice.imageUrl = await compressAndUploadImage(originalChFile);
                            }
            ...
            <div className="d-flex align-items-center gap-2">
                <div className="btn-group">
                    ...
                    <ul className="dropdown-menu">
                        <li><button className="dropdown-item" onClick={handleDownloadTxtTemplate}>Mẫu TXT</button></li>
