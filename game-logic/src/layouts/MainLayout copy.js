...
const Navbar = () => {
  ...
  // Logic tìm slug của giáo viên để điều hướng đến trang dashboard
  // Phần này cần được hoàn thiện thêm nếu bạn muốn nút "Quản lý" trên navbar
  const teacherDashboardLink = user ? `/dashboard/your-slug-here` : '/';

  return (
    <nav className="navbar navbar-expand-lg navbar-light shadow-sm" style={{ backgroundColor: '#ecf0f1' }}>
      <div className="container-fluid px-md-5"> {/* Mở rộng container */}
        ...
        <div className="d-flex align-items-center">
          {user && (
            <>
              ...
              {/* <Link to={teacherDashboardLink} className="btn btn-outline-primary btn-sm me-2">Quản lý</Link> */}
...
const Footer = () => {
  ...
  useEffect(() => {
    // Thiết lập một interval để thay đổi câu nói mỗi 5 giây
  ...
  return (
    <footer className="footer text-center mt-auto py-3" style={{ backgroundColor: '#ecf0f1' }}>
      <div className="container" style={{ height: '2rem' }}> {/* Giúp footer không bị giật khi text thay đổi */}
        {/* Thuộc tính `key` giúp React "reset" animation fadeIn mỗi khi câu nói thay đổi */}
        <span key={quoteIndex} className="motivational-text">
